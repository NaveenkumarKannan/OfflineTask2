...
import com.naveenkumar.offlinetask2.swiperefresh.progress_bar.BaseProgressBar
...
class IGRefreshLayout @JvmOverloads constructor(
    ...
    var DRAG_MAX_DISTANCE = 95
    private val MAX_OFFSET_ANIMATION_DURATION = 300
    ...
    private var mBar: BaseProgressBar = CustomViewProgressBar(context)
    ...
    private fun setupAttributes(attrs: AttributeSet?){
        ...
        DRAG_MAX_DISTANCE = typedArray.getInt(R.styleable.IGRefreshLayout_offsetTop, 95)
            ...
            for(i in 0 until childCount){
                ...
                if(child != mBar) {
                    ...
                    if(mTarget?.background == null)
                        ...
                        mTarget?.setBackgroundColor(ContextCompat.getColor(context, R.color.colorPrimary))
    ...
    private fun setTargetOffsetTop(offset: Int){
        ...
        mCurrentOffsetTop = 0*/
        Utility.log("setTargetOffsetTop: offset = $offset")
        Utility.log("setTargetOffsetTop: mTarget?.Top!! = ${mTarget?.top!!}")
    }
    ...
    private fun moveToStart(interpolatedTime: Float) {
        var targetTop = mFrom - (mFrom * interpolatedTime).toInt()
        val targetPercent = mFromDragPercent * (1.0f - interpolatedTime)
        val offset = targetTop - mTarget?.top!!
        Utility.log("moveToStart: targetTop = $targetTop")
        //targetTop = 0
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        if(customView!=null) {
            if(DRAG_MAX_DISTANCE - customViewHeight <= 0)
                ...
                //customView?.layout(0, 0, 0, 0)
                customView?.layout(left, 0, left + width - right, customViewHeight+dp2px(50))
            else {
                ...
                //customView?.layout(center, 0, customViewWidth+center, DRAG_MAX_DISTANCE - diff/2)
            }
        }
    }
